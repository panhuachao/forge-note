...
- 🕒 **笔记版本管理**：自动留存（停编 3 分钟）+ 手动打点 + 版本比对 diff + 一键恢复，误操作可回溯。
- 🩺 **知识库体检**：无需配置 AI 即可运行，检查失效双链 / 重复内容 / 孤立笔记等 7 类问题，给出健康分并一键整理。
- ✏️ **编辑区 AI 右击菜单**：选中文本右键即可「改写 / 续写 / 翻译」，AI 即时处理，无需切换对话框。
- 🌐 **外部网络搜索**：接入 open-websearch 等 MCP 工具，AI 对话与智能体可实时检索联网信息，补足本地资料。
- ⏱ **流式 AI 回复**：笔记对话与智能体回复逐 token 实时呈现，无需等待整段生成。
- 🔌 **插件系统**：以插件形式扩展锦囊笔记能力，插件可访问笔记预览等能力。
- 🤖 **AI 批量任务**：批量修改 / 移动 / 打标签，执行前逐一预览差异、执行后支持整体回滚；所有 AI 改动均「先建议、后确认」才落盘。
...
| 笔记版本管理 | 自动留存（停编 3 分钟）+ 手动打点 + 版本比对 diff + 一键恢复，误操作可回溯 |
| 知识库体检 | 检查失效双链/重复内容/孤立笔记等 7 类问题，给出健康分并一键整理 |
| 编辑区 AI 右击菜单 | 选中文本右键「改写 / 续写 / 翻译」，AI 即时处理 |
| 外部网络搜索 | 接入 open-websearch 等 MCP，AI 对话与智能体可实时联网检索 |
| 插件系统 | 插件模块 + 管理页面，可扩展笔记预览等能力 |
| AI 批量任务 | 批量修改/移动/打标签，差异预览 + 整体回滚；AI 改动先建议后确认 |
...
- **AI 右击菜单**：选中文本后右键，可「改写 / 续写 / 翻译」，由 AI 基于上下文即时处理。
- **笔记版本管理**：属性面板「版本」入口可查看历史版本；停止编辑 3 分钟自动留存，亦支持手动打点与版本间逐行 diff，一键恢复且恢复本身可撤销。
...
- **流式回复**：笔记侧栏的普通问答与智能体回复均逐 token 实时呈现，边生成边显示，无需等待整段返回。
- **外部网络搜索**：接入 open-websearch 等 MCP 工具后，AI 对话与智能体可实时检索联网信息，补足本地知识库之外的资料。
...
以插件形式扩展锦囊笔记的能力，插件可在受控沙箱内访问笔记预览等能力，围绕笔记内容开发扩展功能：

- **插件管理页面**：统一管理已安装插件的启用 / 禁用与配置，信息展示与操作清晰。
- **笔记预览开放权限**：插件可访问笔记预览，便于实现基于笔记内容的增强功能。
- **实时预览**：源内容变更时，插件涉及的预览可实时刷新。
...
- ✅ V1.2 安全与可靠性：笔记版本管理、知识库体检、AI 批量任务、AI 修改「先建议后确认」、执行后验证与回滚、会话持久化
- ✅ V1.3 扩展与体验：插件系统、笔记编辑区 AI 右击菜单、外部网络搜索 MCP、笔记 AI 对话流式回复、图片轮播
- 🔜 V1.4：万级文件性能优化、PDF/TXT 解析、PWA 移动端、更多官方模板与插件生态
