  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-ink-50">
      <div className="h-10 flex items-center px-4 border-b border-ink-200 bg-white text-sm">
        <span className="font-medium flex items-center gap-1.5"><Icon name="cog" className="w-4 h-4 text-brand-600" /> 设置</span>
      </div>
      <div className="flex-1 overflow-y-auto p-6 space-y-6 max-w-3xl">
        <section className="bg-white rounded border border-ink-200 p-5">
          ...
          <div className="space-y-3 text-sm">
            <div>
              <label className="text-xs text-ink-500">服务类型</label>
              <select
                value={cfg.provider}
            ...
            {cfg.provider === 'ollama' && (
              <>
                <div>
                  <label className="text-xs text-ink-500">服务地址</label>
                  <input
                ...
                <div>
                  <label className="text-xs text-ink-500">模型名称（如 qwen2.5:7b, llama3.1）</label>
                  <input
                    value={cfg.model || ''}
              ...
              <>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="sk-..."
                  />
                  <p className="text-xs text-ink-400 mt-1">API Key 通过系统安全存储加密保存，不会上传。</p>
                </div>
              </>
            )}
        ...
        {activeKb && (
          <section className="bg-white rounded border border-ink-200 p-5">
            ...
            <p className="text-xs text-ink-500 mb-2">当前：{activeKb.name}</p>
            <p className="text-xs text-ink-500 break-all">路径：{activeKb.rootPath}</p>
          </section>
        )}
        ...
        <section className="bg-white rounded border border-ink-200 p-5">
          ...
          <p className="text-sm text-ink-600">锦囊笔记 ForgeNote V1.1</p>
          <p className="text-xs text-ink-500 mt-1">MIT License · Forge your knowledge.</p>
        </section>
      </div>
    </div>
  );
